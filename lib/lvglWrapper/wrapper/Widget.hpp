...
namespace lvgl
{
    ...
    protected:
        lv_obj_t *_obj;
        inline void addEvent(lv_event_cb_t event_cb, lv_event_code_t event_type = LV_EVENT_ALL)
        {
    ...
    public:
        ...
        inline That &addStyle(lv_style_t *style, lv_style_selector_t selector = LV_PART_MAIN)
        {
            lv_obj_add_style(_obj, style, selector);
            return static_cast<That &>(*this);
        }

        inline That &addStyle(Style *style, lv_style_selector_t selector = LV_PART_MAIN)
        {
