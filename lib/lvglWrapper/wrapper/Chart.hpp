...
public:
  ...
  Chart(lv_chart_type_t type, Widget *parent) { Chart(type, parent->get()); }
  ...
  inline uint16_t getPointCount() { return lv_chart_get_point_count(_obj); }
  ...
  inline Chart &setAxisTick(lv_chart_axis_t axis, uint16_t major_len,
                            uint16_t minor_len, uint16_t major_cnt,
                            uint16_t minor_cnt, bool label_en,
                            uint16_t draw_size) {
    lv_chart_set_axis_tick(_obj, axis, major_len, minor_len, major_cnt,
                           minor_cnt, label_en, draw_size);
    return *this;
  }

  inline void refresh() { lv_chart_refresh(_obj); }
};
} // namespace lvgl